...
export default function InvitePage() {
  ...
  const handleSend = async () => {
    if (!name || !email || !role) { setError("Please fill in all fields"); return }
    setLoading(true)
    setError("")
    try {
        ...
        {sent ? (
          <div style={{ backgroundColor: '#ffffff', borderRadius: '20px', padding: '48px 32px', textAlign: 'center', border: '1px solid #f1f5f9', boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ backgroundColor: '#f8fafc', borderRadius: '12px', padding: '16px', marginBottom: '24px', textAlign: 'left' }}>
              ...
              <p style={{ fontSize: '13px', color: '#64748b' }}>Temporary password: <span style={{ fontFamily: 'monospace', color: '#0f172a', fontWeight: 600 }}>{temporaryPassword}</span></p>
              <p style={{ fontSize: '11px', color: '#94a3b8', marginTop: '8px' }}>Share these credentials with {name} securely. They'll be asked to set a new password on first login.</p>
            </div>
            <button onClick={() => { setSent(false); setName(""); setEmail(""); setTemporaryPassword(""); }}
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '20px', padding: '32px', border: '1px solid #f1f5f9', boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
            ...
            <p style={{ fontSize: '13px', color: '#64748b', marginBottom: '24px' }}>A temporary password will be generated for them — share it securely after creating the account.</p>
