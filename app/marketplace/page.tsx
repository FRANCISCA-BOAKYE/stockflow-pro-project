...
const [listings, setListings] = useState(mockBusinesses)
    ...
    .then(data => {
      if (Array.isArray(data) && data.length > 0) {
        setListings(data.map((item: any) => ({
          id: String(item.id),
          name: item.businessName || item.headline,
          type: item.tierType?.toLowerCase() || 'manufacturer',
          tier: 'standard',
          products: [],
          priceRange: item.creditTerms || 'Contact for pricing',
          deliveryTerms: item.deliveryTerms || 'TBD',
          creditTerms: item.creditTerms || 'TBD',
          minOrderQuantity: 'Contact seller',
