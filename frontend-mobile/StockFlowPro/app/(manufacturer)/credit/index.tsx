...
const INITIAL_THEY_OWE_ME = [
  { id: '1', name: 'Apex Distributors', due: 'Jun 30, 2026', amount: 42000.00, status: 'DUE_SOON', held: false },
  { id: '2', name: 'Sunrise Wholesale', due: 'Jun 15, 2026', amount: 68000.00, status: 'OVERDUE', held: false },
  { id: '3', name: 'Delta Trading Co', due: 'Jul 10, 2026', amount: 14800.00, status: 'OUTSTANDING', held: false },
  { id: '4', name: 'Metro Distributors', due: 'May 30, 2026', amount: 22000.00, status: 'SETTLED', held: false },
];
...
export default function ManufacturerCreditScreen() {
  ...
  const toggleHold = (id: string) => {
    ...
    Alert.alert(
      ...
      acct.held
        ? `Remove the credit hold on ${acct.name}? They will be able to make new credit purchases again.`
        : `Place a hold on ${acct.name}? This blocks new credit transactions with them until the balance is cleared.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: acct.held ? 'Remove hold' : 'Place hold',
          style: acct.held ? 'default' : 'destructive',
          onPress: () => setTheyOweMe(prev => prev.map(a => a.id === id ? { ...a, held: !a.held } : a))
        }
      ]
    );
  };
