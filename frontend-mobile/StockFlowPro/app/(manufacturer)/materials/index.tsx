...
export default function MaterialsScreen() {
  ...
  const handleAddMaterial = async () => {
    ...
    if (!form.quantity) errors.quantity = 'Initial quantity is required.';
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={s.empty}>
              ...
              <TouchableOpacity style={s.emptyActionBtn} onPress={() => { setLastAdded(''); setShowAddModal(true); }}>
                <Ionicons name="add" size={16} color={colors.onPrimary} />
                <Text style={s.emptyActionText}>Add Material</Text>
              </TouchableOpacity>
            </View>
          }
      ...
      <Modal visible={showActionsModal} animationType="slide" transparent onRequestClose={() => setShowActionsModal(false)}>
        <TouchableOpacity style={s.sheetBackdrop} activeOpacity={1} onPress={() => setShowActionsModal(false)}>
          <TouchableOpacity activeOpacity={1} style={s.sheet}>
            ...
            <TouchableOpacity style={s.sheetAction} onPress={async () => {
              const material = selectedMaterial;
              setShowActionsModal(false);
              const ok = await confirm({
                title: 'Delete material',
                message: `Remove "${material?.name}"? This cannot be undone.`,
                destructive: true,
                confirmLabel: 'Delete',
                icon: 'trash-outline',
              });
              if (ok) handleDeleteMaterial(material.id);
            }}>
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: colors.surface }}>
          ...
          <ScrollView contentContainerStyle={s.modalBody}>
            ...
            {[
              ...
              { label: 'Cost per unit', key: 'costPerUnit', placeholder: '2.50', keyboard: 'decimal-pad' },
            ].map((field, i) => (
              <View key={field.key}>
                {i === 1 && (
                  <>
                    <UnitPicker value={form.unit} onChange={v => { setForm(f => ({ ...f, unit: v })); if (fieldErrors.unit) setFieldErrors(e => ({ ...e, unit: '' })); }} label="Unit *" />
                    {fieldErrors.unit ? <Text style={s.fieldError}>{fieldErrors.unit}</Text> : null}
                  </>
                )}
                <View style={{ marginBottom: 16 }}>
                  ...
                  {field.key === 'costPerUnit' ? (
                    <View style={s.priceInputRow}>
                      <Text style={s.priceSymbol}>{country.currencySymbol}</Text>
                      <TextInput
                        style={s.priceInput}
                    ...
                    <TextInput
                      ...
                      onChangeText={v => {
                        ...
                        if (fieldErrors[field.key]) setFieldErrors(e => ({ ...e, [field.key]: '' }));
                      }}
                      keyboardType={(field.keyboard as any) || 'default'}
                    />
                  )}
                  {fieldErrors[field.key] ? <Text style={s.fieldError}>{fieldErrors[field.key]}</Text> : null}
                </View>
              </View>
            ))}
            ...
            {stockInMode === 'package' && selectedMaterial?.packageUnit ? (
              <>
                ...
                <View style={s.stepperRow}>
                  <TouchableOpacity style={s.stepBtn} onPress={() => setStockInPackages(p => String(Math.max(0, (parseInt(p || '0', 10) || 0) - 1)))}>
                    <Ionicons name="remove" size={18} color={colors.textSecondary} />
                  </TouchableOpacity>
                  <Text style={s.stepNum}>{stockInPackages || '0'}</Text>
                  <TouchableOpacity style={[s.stepBtn, s.stepBtnBlue]} onPress={() => setStockInPackages(p => String((parseInt(p || '0', 10) || 0) + 1))}>
...
const makeStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  priceInputRow: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: colors.borderStrong, borderRadius: 12, backgroundColor: colors.surfaceAlt },
  priceSymbol: { paddingLeft: 12, color: colors.textMuted, fontWeight: '600' },
  priceInput: { flex: 1, padding: 12, fontSize: 14, color: colors.textPrimary },
