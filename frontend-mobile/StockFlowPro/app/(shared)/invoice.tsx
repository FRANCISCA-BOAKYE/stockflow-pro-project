...
export default function InvoicesScreen() {
  ...
  const filtered = filter === 'ALL' ? invoices : invoices.filter(i => (i.status || (i.paymentMode === 'CREDIT' ? 'UNPAID' : 'PAID')) === filter);
  const total = filtered.reduce((sum, i) => sum + Number(i.totalUsd || 0), 0);
