...
const WHOLESALERS = [
  { id: '1', name: 'Apex Distributors', location: 'Accra, Ghana', products: ['Beverages', 'Dry Goods'], rating: 4.6, verified: true, linked: true, creditLimit: 10000 },
  { id: '2', name: 'Metro Wholesale', location: 'Tema, Ghana', products: ['Cement', 'Steel'], rating: 4.3, verified: false, linked: true, creditLimit: 5000 },
  { id: '3', name: 'Volta Distributors', location: 'Ho, Ghana', products: ['Electronics'], rating: 4.9, verified: true, linked: false, creditLimit: 0 },
...
export default function LinkedWholesalersScreen() {
  ...
  const toggleLink = (id: string) => {
    ...
    if (!w) return;
    Alert.alert(w.linked ? 'Unlink' : 'Link', `${w.linked ? 'Unlink from' : 'Send link request to'} ${w.name}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: w.linked ? 'Unlink' : 'Send', style: w.linked ? 'destructive' : 'default', onPress: () => setWholesalers(prev => prev.map(x => x.id === id ? { ...x, linked: !x.linked } : x)) }
    ]);
  };
