      ...
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState
            ...
            message="Enter a buyer's phone number at POS to start building their customer ID"
          />
        }
        renderItem={({ item }) => (
          <ListItemCard
            ...
            title={item.name}
            subtitle={`ID #${item.id} · ${item.phone || 'no phone'} · ${item.purchaseCount} purchase${item.purchaseCount !== 1 ? 's' : ''}`}
            trailing={
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <ListItemCard
                  ...
                  subtitle={`${item.paymentMode} · ${new Date(item.recordedAt).toLocaleDateString()}`}
                  trailing={<Text style={s.historyAmt}>{format(Number(item.amountUsd))}</Text>}
                />
              )}
            />
          )}
...
const makeStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  spent: { fontSize: 13, fontWeight: '700', color: colors.success, fontVariant: ['tabular-nums'] },
