...
export default function WholesalerCreditScreen() {
  ...
  const theyOweMe = accounts.filter(a => a.status !== 'SETTLED');
  const iOweThem = accounts.filter(a => a.status !== 'SETTLED');
  const data = tab === 'owe_me' ? theyOweMe : iOweThem;
  const total = data.reduce((sum, a) => sum + Number(a.amountUsd), 0);
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={s.empty}>
              ...
              <Text style={s.emptySub}>Credit accounts appear when you sell on credit</Text>
            </View>
          }
          renderItem={({ item }) => {
      ...
      <Modal visible={showPaymentModal} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setShowPaymentModal(false)}>
        <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
          ...
          <View style={{ padding: 16 }}>
            ...
            <TextInput style={s.fieldInput} placeholder="Enter amount" placeholderTextColor="#9CA3AF"
              value={paymentAmount} onChangeText={setPaymentAmount} keyboardType="decimal-pad" />
