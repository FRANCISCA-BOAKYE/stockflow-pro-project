      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={s.empty}>
            ...
            <Text style={s.emptyText}>No linked partners</Text>
            <Text style={s.emptySub}>Tap Link to connect with manufacturers or retailers</Text>
          </View>
        }
        renderItem={({ item }) => {
      ...
      <Modal visible={showRequestModal} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setShowRequestModal(false)}>
        <SafeAreaView style={{ flex: 1, backgroundColor: colors.surface }}>
          <View style={s.modalHeader}>
            <Text style={s.modalTitle}>Link with Partner</Text>
            <TouchableOpacity onPress={() => setShowRequestModal(false)}>
              <Ionicons name="close" size={24} color={colors.textSecondary} />
            </TouchableOpacity>
          </View>
          <View style={{ padding: 16 }}>
            <Text style={s.fieldLabel}>Business ID</Text>
            <TextInput style={s.fieldInput} placeholder="e.g. 1" placeholderTextColor={colors.textPlaceholder}
